...

{
  ...
  test = ''
    ...
    grep "sysadminctl -addUser ${lib.escapeShellArgs [ "created.user" "-UID" 42001 ]}" ${config.out}/activate
    (! grep "sysadminctl -addUser ${lib.escapeShellArg "created.user"} .* -home" ${config.out}/activate)
    (! grep "sysadminctl -addUser ${lib.escapeShellArg "created.user"} .* -shell" ${config.out}/activate)
    ...
    grep "dscl . -create ${lib.escapeShellArg "/Users/foo"} NFSHomeDirectory ${lib.escapeShellArg "/Users/foo"}" ${config.out}/activate
    grep "createhomedir -cu ${lib.escapeShellArg "foo"}" ${config.out}/activate
    ...
    (! grep "dscl . -create ${lib.escapeShellArg "/Users/created.user"} NFSHomeDirectory" ${config.out}/activate)
